...
public:
    constexpr XDOM(List<DOM>* lst, i32 idx) noexcept
        : pnodes_(lst)
        , index_(idx)
    {}
#pragma endregion
...
#pragma region format
    void format(String<>& buf, const StrView& fmt) const {
        if (fmt == "xml") {
            format(buf, $xml);
        }
        else {
    ...
    void format(String<>& buf, DOMType type) const {
        switch (type) {
        case $json:
            _format_json(buf);
            break;
...
private:
    NMS_API void _format_json(String<>& buf, u32 level = 0) const;

    NMS_API void _format_xml_begin(String<>& buf) const;
    NMS_API void _format_xml_end  (String<>& buf) const;
    NMS_API void _format_xml      (String<>& buf, u32 level = 0) const;

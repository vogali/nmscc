...
nms_test(file) {
    ...
    // write
    {
        for (u32 i = 0; i < data_in.$capicity; ++i) {
            data_in += i;
        }
    ...
    // read
    {
        ...
        test::assert_true(file.size() == data_out.$capicity*sizeof(u32));
        file.read(data_out.data(), data_out.$capicity);
        data_out.resize(data_out.$capicity);

        for (u32 i = 0; i < data_out.$capicity; ++i) {

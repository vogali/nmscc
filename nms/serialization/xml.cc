...
NMS_API void XDOM::_format_xml_begin(String<>& buf) const {
    const auto& v = val();
    buf += "<xml? version=\"1.0\" encoding=\"utf-8\" ?>\n";
    sformat(buf, "<root type={}>", v.type());
}

NMS_API void XDOM::_format_xml_end(String<>& buf) const {
    buf += "</root>";
}

NMS_API void XDOM::_format_xml(String<>& buf, u32 level) const {
